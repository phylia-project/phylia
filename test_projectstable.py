...
import pytest
import pandas as pd
from phylia.tools import ProjectsTable
...
def test_getprojectnames(ptable):
    result = ptable.get_projectnames()
    assert(isinstance(result, pd.DataFrame))
    assert not result.empty


def get_projectfolders(ptable):
    result = ptable.get_projectfolders(filetype='.mdb')
    assert(isinstance(result,pd.DataFrame))
    assert not result.empty

    result = ptable.get_projectfolders(filetype='.shp')
    assert(isinstance(result,pd.DataFrame))
    assert not result.empty
...
def test_projectfiles_counts(ptable):
    table = ptable.list_projectfiles(filetype='.mdb')
...
def test_filter_mdbfiles(ptable):
    ...
    mdblist = ptable.get_mdbfileslist_files('mdb')
