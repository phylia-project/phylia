...
from . import _synbiosys_vvn

def rvvn_syntables():
    """Presence and fidelity of species in syntaxa within the rvvn system."""
    srcfile = (_resources.files(_synbiosys_vvn) / 'synbiosys_syntaxa_tabellen2017.csv')
...
def rvvn_syntaxa():
    ...
    srcfile = (_resources.files(_synbiosys_vvn) / 'synbiosys_syntaxa_2017.csv')
...
def rvvn_statistics():
    ...
    srcfile = (_resources.files(_synbiosys_vvn) / 'synbiosys_syntaxa_metadata2017.csv')
    return _pd.read_csv(srcfile, encoding='latin-1')


def species_2017():
    """Return species list 'soorten_2017'."""
    srcfile = (_resources.files(_synbiosys_vvn) / 'synbiosys_soorten_2017.csv')
